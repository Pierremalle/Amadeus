...
// Composant principal MusicCards
export default function MusicCards({ song }) {
    // Utilisation de useState pour gérer l'état "aimé"
    const [liked, setLiked] = useState(false);
    // État pour gérer la lecture (pause/play)
    const [isPlaying, setIsPlaying] = useState(false);

    // État pour le temps actuel de lecture
    ...
    const progressPercentage = Math.floor((currentTime / song.duration) * 100);
    ...
    // Effet pour initialiser l'élément audio
    useEffect(() => {
        // Créer un élément audio s'il n'existe pas déjà
        if (!audioRef.current) {
            audioRef.current = new Audio(song.path);
            console.log(audioRef.current.src)

            // Mettre à jour la durée quand les métadonnées sont chargées
            audioRef.current.onloadedmetadata = () => {
                if (audioRef.current) {
                    // Mettre à jour la durée avec la durée réelle du fichier audio
                    const actualDuration = Math.floor(audioRef.current.duration);
                    if (actualDuration && !isNaN(actualDuration)) {
                        song.duration = actualDuration;
                    }
                }
            };
    ...
    // Effet pour gérer les changements de source audio
    useEffect(() => {
        if (audioRef.current && audioRef.current.src !== song.path) {
            audioRef.current.src = song.path;
            audioRef.current.load();
        }
    }, [song.path]);

    // Effet pour gérer la lecture/pause
    useEffect(() => {
        if (audioRef.current) {
            if (isPlaying) {
                const playPromise = audioRef.current.play();

                // Gérer la promesse pour éviter les erreurs
    ...
    const handleProgressChange = (value) => {
        const newTime = Math.floor((value / 100) * song.duration);
        setCurrentTime(newTime);

        // Mettre à jour la position de lecture dans l'élément audio
        if (audioRef.current) {
                    ...
                    <div className="flex flex-col col-span-6 md:col-span-8">
                        <div className="flex justify-between items-start">
                            <div className="flex flex-col gap-0">
                                <h3 className="font-semibold text-foreground/90">{song.name}</h3>
